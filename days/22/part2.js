...
const originalBricks = data.split('\n').map(line => {
...
const bricks = originalBricks.sort((a, b) => a.z1 - b.z1);
...
const result = originalBricks
    .reduce((a, b) => {
        ...
        while(queue.length){
            ...
            queue.push(...current.supports.filter(e => e.isSupportedBy.every(p => seen.has(p))));
        }

        return a + seen.size - 1;
    }, 0)
